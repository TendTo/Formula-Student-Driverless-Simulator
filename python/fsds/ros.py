...
class RosBridgeClient:
    ...
    @classmethod
    def dataclass_to_ros_msg(
        cls, msg: "Vector3r | Quaternionr | Pose | GeoPoint | ImageResponse"
    ) -> dict:
        ...
        if isinstance(msg, ImuData):
            ...
            return {
                ...
                "linear_acceleration": cls.dataclass_to_ros_msg(
                    msg.linear_acceleration
                ),
                "angular_velocity_covariance": [avc, 0, 0, 0, avc, 0, 0, 0, avc],
                "linear_acceleration_covariance": [lac, 0, 0, lac, 0, 0, 0, lac],
            }
        if isinstance(msg, LidarData):
            ...
            return {
                "header": Header(),
                "height": 1,
                "width": len(msg.point_cloud),
                "fields": fields,
                "is_bigendian": False,
                "point_step": offset,
                "row_step": offset * len(msg.point_cloud),
                "is_dense": True,
                "data": msg.point_cloud
            }
        return msg.to_dict()
    ...
    def publish(
        ...
        Topic(self._client, topic, obj_to_msg_type[type(msg)], latch=True).publish(
            self.dataclass_to_ros_msg(msg)
        )
    ...
    def subscribe(
        self, topic: str, msg_type: type[T], cb: "Callable[[T], None]"
    ) -> int:
        self._subscribed_topics.append(
            Topic(self._client, topic, obj_to_msg_type[msg_type])
        )
        self._subscribed_topics[-1].subscribe(
            lambda msg: cb(self.ros_msg_to_dataclass(msg_type, msg))
        )
